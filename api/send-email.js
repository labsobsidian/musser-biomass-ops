// /api/send-email.js
//
// Compatibility endpoint for Biomass Buddy's existing "Email CEO" actions.
...
import { isGhlConfigured, sendGhlEmail } from '../lib/connectors/ghl.js';
...
function normalizeAttachments(attachments) {
  ...
  return attachments
    ...
    .map((item, index) => {
      const filename = sanitizeFilename(item.filename || `biomass-buddy-report-${index + 1}.pdf`);
      ...
      if (item.contentText) {
        return {
          ...
          content: createPdfBuffer({
            title: item.title || 'Biomass Buddy Report',
...
function sanitizeFilename(filename) {
  return String(filename)
    ...
    .replace(/^-+|-+$/g, '')
    .slice(0, 90) || 'biomass-buddy-report.pdf';
}
...
function createPdfBuffer({ title, text }) {
  ...
  const maxLines = 48;
  const wrappedLines = [
    String(title || 'Biomass Buddy Report'),
    '',
...
function createArPdfBuffer(data) {
  ...
  drawText(p1, 'BIOMASS BUDDY AR BRIEF', 422, 742, 9, 'F2', '#FFFDF8');
  ...
  drawText(p1, 'Generated by Biomass Buddy for Musser Biomass', 48, 34, 8, 'F1', '#8A7B67');
  pages.push(p1.join('\n'));
