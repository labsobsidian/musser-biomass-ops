...
function createArPdfBuffer(data) {
  ...
  drawText(p1, 'Accounts Receivable', 48, 604, 30, 'F2', '#17130F');
  ...
  drawRect(p1, 48, 360, 516, 96, '#F7F3EA');
  drawRect(p1, 48, 360, 5, 96, '#D19A2E');
  drawText(p1, 'ATLAS READ - OWNER ATTENTION', 64, 430, 8, 'F2', '#1F6B4A');
  drawWrappedText(p1, data.aiSummary || 'No AR summary available.', 64, 408, 78, 12, 'F2', '#2B241D', 17);

  drawText(p1, 'Aging Summary', 48, 320, 18, 'F2', '#17130F');
  ...
  drawRect(p1, 48, 64, 516, 84, '#173D2B');
  drawText(p1, 'COLLECTION PRIORITY', 64, 124, 8, 'F2', '#D8B35A');
  drawWrappedText(p1, priority[0] || 'Review overdue accounts before approving additional loads.', 64, 104, 70, 11, 'F2', '#FFFDF8', 15);
  ...
  drawText(p2, 'Recommended Follow-ups', 48, 238, 18, 'F2', '#17130F');
  let actionY = 210;
