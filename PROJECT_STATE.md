...
6. Seasonal demand swings (firewood peaks Oct–Feb) hit without a plan
...
- `/api/claude` — streaming proxy to Anthropic (reads live KB from this repo)
...
- [ ] Reminder workflow for seasonal customers (firewood pre-buy in September)
...
- [ ] QuickBooks sync for revenue numbers (replace seeded finance data)
- [ ] Inventory tracking in Supabase (cords on hand, mulch yards, bundle counts)
