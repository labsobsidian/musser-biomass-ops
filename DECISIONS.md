...
- AR CEO summaries now send as Musser-branded HTML operating briefs rather than plain-text notes. The email template must carry explicit Musser signals, not just Atlas styling: the repo-root Musser logo served from `/musser-logo.webp`, Musser Biomass & Wood Products, Rural Retreat, Appalachian hardwood fiber, Forest Fuel, Alpha Fiber, and the plant address. It should follow the website/logo feel with warm off-white surfaces, green bands, charcoal type, and restrained gold accents.
